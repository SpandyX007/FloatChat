...
def extract_and_run_pipeline(
    ...
    csv_path: str = r"D:\BMSIT College Docs\SEM5\MiniProject\React_frontend\backendfloatchat\utils\grid_sampled_profiles.csv"
...
def filter_dataframe(df: pd.DataFrame, desired_params: list) -> pd.DataFrame:
    ...
    """
    # Try to include date column if it exists
    keep_cols = []
    
    # Check for date columns in order of preference
    for date_col in ["profile_date", "month", "date"]:
...
def host_process(user_question: str) -> dict:
    ...
    if argo_df is None or argo_df.empty:
        error_response = "No data found matching your query criteria. This could be because:\n" \
                        "1. No ARGO float profiles exist in the specified region and time period\n" \
                        "2. The date range may be outside available data\n" \
                        "3. The region specified may not have ARGO coverage\n\n" \
                        "Please try:\n" \
                        "- Expanding the date range\n" \
                        "- Adjusting the geographic region\n" \
                        "- Using a broader query"
        return error_response, pd.DataFrame()
    ...
    if argo_df.empty:
        error_response = "Data was found but no matching parameters. Please check if the parameters you requested are available in the dataset."
        return error_response, pd.DataFrame()
